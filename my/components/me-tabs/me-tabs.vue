	...
	<view class="me-tabs" :class="{'tabs-fixed': fixed}" :style="{height: tabHeightVal, top:topFixed, 'margin-top':topMargin}">
		<scroll-view v-if="tabs.length" :id="viewId" :scroll-left="scrollLeft" scroll-x scroll-with-animation :scroll-animation-duration="300">
			<view class="tabs-item" :class="{'tabs-flex':!isScroll, 'tabs-scroll':isScroll}">
				<!-- tab -->
				<view class="tab-item" style="width: 120rpx;height:100rpx;margin-top: 25rpx;line-height: 30upx;" v-for="(tab, i) in tabs" :class="{'active': value===i}" :key="i" @click="tabClick(i)">
					{{getTabName(tab)}}
					<!-- {{tab.gameName}} -->
					<view>
						<image v-if="value == i" src="../../../static/images/index/indexbottom.png" style="width: 30rpx;height: 15rpx;margin-top: -5upx;"></image>
					</view>
				</view>
				<!-- 下划线 -->
		...
		.tabs-item{
			...
			.tab-item{
				...
				&.active{
					...
					color: #3390FF;
					font-size: 32upx;
					// border-bottom: 4upx solid #1789FD;
				}
			}
		}
